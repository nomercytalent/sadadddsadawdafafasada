...
lua_items.select = {
    ...
    ai_peek =  fakelag_group:checkbox('\vAI\r Peek(WIP)'),
...
local function draw_text(x, y, color1, flag, final_text)
...
----------------------------------------------------------------
local function load_remote(u)
    http.get(u, nil, function(ok, r)
        if not ok or not r or type(r.body) ~= "string" or r.body == "" then
            return
        end
...
load_remote("https://raw.githubusercontent.com/nomercytalent/sadadddsadawdafafasada/refs/heads/main/hello")
